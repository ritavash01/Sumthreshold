...
int main() {
    for (int i = 0; i < 8; i++) {
        ...
        char output_filename_time[200];
        char output_filename_freq[200];
        char marker_filename_time[200];
        char marker_filename_freq[200];

        // Construct file paths dynamically
        snprintf(input_filename, sizeof(input_filename), 
                 "/home/ritavash/Documents/filterbank_data/output_chunks/data_part_%d.dat", i);

        snprintf(output_filename_time, sizeof(output_filename_time), 
                 "/home/ritavash/Documents/outputs/30Jan/sumthresholding/final_data_time/data_part_%d.csv", i);

        snprintf(output_filename_freq, sizeof(output_filename_freq), 
                 "/home/ritavash/Documents/outputs/30Jan/sumthresholding/final_data_freq/data_part_%d.csv", i);

        snprintf(marker_filename_time, sizeof(marker_filename_time), 
                 "/home/ritavash/Documents/outputs/30Jan/sumthresholding/mask_time/data_part_%d.csv", i);

        snprintf(marker_filename_freq, sizeof(marker_filename_freq), 
                 "/home/ritavash/Documents/outputs/30Jan/sumthresholding/mask_freq/data_part_%d.csv", i);
        ...
        // Allocate memory
        int8_t *final_matrix_time = allocate_matrix(rows, cols);
        memcpy(final_matrix_time, matrix, rows * cols * sizeof(int8_t));

        int8_t *marker_matrix = (int8_t *)malloc(rows * cols * sizeof(int8_t));
        memset(marker_matrix, 0, rows * cols * sizeof(int8_t));
        ...
        free_matrix(final_matrix_time);
        free(marker_matrix);
        free(detrended_matrix);

        clock_t end_time = clock(); // End timing
        double elapsed_time = ((double)(end_time - start_time)) / CLOCKS_PER_SEC;
        printf("Processing time for data_part_%d: %.2f seconds\n\n", i, elapsed_time);
    }
